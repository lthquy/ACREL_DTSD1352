...
ACREL_DTSD1352::EnergyData ACREL_DTSD1352::readEnergyBatch() {
  ...
  delay(20); // Delay nhỏ giữa các lần đọc
...
ACREL_DTSD1352::THDData ACREL_DTSD1352::readTHDBatch() {
  ...
  data.thdVoltageA = buffer[0] * 0.001;
  data.thdVoltageB = buffer[1] * 0.001;
  data.thdVoltageC = buffer[2] * 0.001;
  data.thdCurrentA = buffer[3] * 0.001;
  data.thdCurrentB = buffer[4] * 0.001;
  data.thdCurrentC = buffer[5] * 0.001;
...
// Đọc Harmonic bậc N (Dynamic)
// Tính địa chỉ dựa trên bậc n
// Giả định: Harmonic data bắt đầu từ 0x05E3, mỗi bậc cách nhau 30 thanh ghi 
ACREL_DTSD1352::HarmonicNData ACREL_DTSD1352::readHarmonicN(uint8_t n) {
  HarmonicNData data;
  data.order = n;
  data.valid = false;
  
  // Validate input (chỉ chấp nhận bậc 2-31)
  if (n < 2 || n > 31) {
    Serial.printf("[ERROR] readHarmonicN: Invalid order %d (only 2-31 allowed)\n", n);
    return data;
  }
  
  // Tính địa chỉ base cho bậc n 2-31

  uint16_t baseAddr = 0x05E1 + n;
  
  Serial.printf("[READ] Reading Harmonic %d (base addr: 0x%04X)...\n", n, baseAddr);
  
  // Đọc Voltage A
  uint32_t hnVa = readHoldingRegisterSigned(baseAddr);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Voltage A failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  delay(10);
  
  // Đọc Voltage B
  uint32_t hnVb = readHoldingRegisterSigned(baseAddr + 30);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Voltage B failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  delay(10);
  
  // Đọc Voltage C
  uint32_t hnVc = readHoldingRegisterSigned(baseAddr + 60);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Voltage C failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  delay(10);
  
  // Đọc Current A
  uint32_t hnIa = readHoldingRegisterSigned(baseAddr + 90);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Current A failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  delay(10);
  
  // Đọc Current B
  uint32_t hnIb = readHoldingRegisterSigned(baseAddr + 120);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Current B failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  delay(10);
  
  // Đọc Current C
  uint32_t hnIc = readHoldingRegisterSigned(baseAddr + 150);
  if (_lastError != 0) {
    Serial.printf("[ERROR] readHarmonicN(%d): Current C failed, error=0x%02X\n", n, _lastError);
    return data;
  }
  
  // Convert (đơn vị 0.001%)
  data.voltageA = hnVa * 0.001;
  data.voltageB = hnVb * 0.001;
  data.voltageC = hnVc * 0.001;
  data.currentA = hnIa * 0.001;
  data.currentB = hnIb * 0.001;
  data.currentC = hnIc * 0.001;
  
  Serial.printf("[OK] readHarmonicN(%d): Success\n", n);
  data.valid = true;
  return data;
}
